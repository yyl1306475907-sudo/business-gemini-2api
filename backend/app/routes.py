...
def register_routes(app):
            ...
            for retry_idx in range(max_retries):
                ...
                try:
                    ...
                    if stream:
                        ...
                        # 在调用生成器之前检测图片格式（因为生成器中 request 上下文不可用）
                        image_format = detect_client_image_format(request, data)
                        print(f"[流式请求] User-Agent: {request.headers.get('User-Agent', '')}")
                        print(f"[流式请求] 检测到图片格式: {image_format}")
                        ...
                        stream_generator = stream_chat_realtime_generator(
                            ...
                            host_url=request.host_url, image_format=image_format
                        )
    ...
    def get_accounts():
        ...
        return jsonify({
            ...
            "current_index": account_manager.last_used_account_index
        })
    ...
    def add_account():
        """添加账号"""
        data = request.json
        print(f"[DEBUG] 收到添加账号请求: {data}")  # 调试日志
        
        new_csesidx = data.get("csesidx", "")
        new_team_id = data.get("team_id", "")
        
        print(f"[DEBUG] team_id: {new_team_id}, csesidx: {new_csesidx}")  # 调试日志
        ...
        # 添加临时邮箱字段（如果提供）
        if "tempmail_name" in data and data["tempmail_name"]:
            new_account["tempmail_name"] = data["tempmail_name"]
        if "tempmail_url" in data and data["tempmail_url"]:
            new_account["tempmail_url"] = data["tempmail_url"]
        
        print(f"[DEBUG] 创建的账号: {new_account}")  # 调试日志
        ...
        if cookie_missing:
            ...
            if auto_refresh_enabled:
                try:
                    ...
                    pass
        elif cookie_updated and secure_c_ses and csesidx:
            # Cookie 更新成功且完整，自动启用账号
            acc["available"] = True
            ...
            print(f"[✓] 账号 {account_id} Cookie 更新成功，已自动启用")
    ...
    @app.route('/api/health-check/status', methods=['GET'])
    @require_admin
    def get_health_check_status():
        """获取健康检查状态"""
        from .account_health_check import get_health_check_status
        status = get_health_check_status()
        # 添加配置信息
        status["enabled"] = account_manager.config.get("health_check_enabled", False)
        status["interval_minutes"] = account_manager.config.get("health_check_interval", 30)
        status["auto_delete"] = account_manager.config.get("health_check_auto_delete", False)
    ...
    def run_health_check_now():
        """立即运行一次健康检查"""
        from .account_health_check import run_health_check
        auto_delete = request.json.get("auto_delete", False) if request.json else False
        results = run_health_check(account_manager, auto_delete)
        return jsonify({
            "success": True,
            "results": results,
            "summary": {
    ...
    def start_health_check():
        ...
        data = request.json or {}
        interval = data.get("interval_minutes", account_manager.config.get("health_check_interval", 30))
        auto_delete = data.get("auto_delete", account_manager.config.get("health_check_auto_delete", False))
        
        # 保存配置
        account_manager.config["health_check_enabled"] = True
        account_manager.config["health_check_interval"] = interval
        account_manager.config["health_check_auto_delete"] = auto_delete
        account_manager.save_config()
        
        start_health_check(account_manager, interval, auto_delete)
        return jsonify({"success": True, "message": f"定时健康检查已启动，间隔 {interval} 分钟"})
    
    @app.route('/api/health-check/stop', methods=['POST'])
    @require_admin
    def stop_health_check():
        """停止定时健康检查"""
        from .account_health_check import stop_health_check
        
        account_manager.config["health_check_enabled"] = False
        account_manager.save_config()
        
        stop_health_check()
        return jsonify({"success": True, "message": "定时健康检查已停止"})
